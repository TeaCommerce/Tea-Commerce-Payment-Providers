...
namespace TeaCommerce.PaymentProviders.Web {
  ...
  public class CyberSource : APaymentProvider {
    ...
    public override PaymentHtmlForm GenerateHtmlForm( Order order, string teaCommerceContinueUrl, string teaCommerceCancelUrl, string teaCommerceCallBackUrl, IDictionary<string, string> settings ) {
      ...
      string[] settingsToExclude = new[] { "form_url", "secret_key", "mode" };
      ...
      htmlForm.InputFields[ "cancel_url" ] = teaCommerceContinueUrl;
      htmlForm.InputFields[ "callback_url"]= teaCommerceCallBackUrl;

      htmlForm.InputFields[ "cart_number" ] =order.CartNumber;
      htmlForm.InputFields[ "amount" ] = order.TotalPrice.WithVat.ToString( "0.00" );
      htmlForm.InputFields[ "currency" ] = CurrencyService.Instance.Get( order.StoreId, order.CurrencyId ).IsoCode;
    ...
    protected CallbackInfo ProcessCallback_Sign( Order order, HttpRequest request, IDictionary<string, string> settings ) {
      ...
      try {
        ...
        OutputJson( new {
          error = false,
          signature_timestamp = signatureTimestamp, signature
        } );
      } catch ( Exception ex ) {
        OutputJson( new {
          error = true,
    ...
    protected CallbackInfo ProcessCallback_Receipt( Order order, HttpRequest request, IDictionary<string, string> settings ) {
      if ( settings[ "mode" ] != "live" )
    ...
    protected String Sign( NameValueCollection paramsArray, string signatureTimestamp, string secretKey ) {
      return Sign( BuildDataToSign( paramsArray.AllKeys.ToDictionary( k => k,
                  k => paramsArray[ k ] ), signatureTimestamp ), secretKey );
    }

    protected String Sign( IDictionary<string, string> paramsArray, string signatureTimestamp, string secretKey ) {
    ...
    protected String BuildDataToSign( IDictionary<string, string> paramsArray, string signatureTimestamp ) {
    ...
    protected void LogCallback( Order order, HttpRequest request, IDictionary<string, string> settings, string fileName ) {
